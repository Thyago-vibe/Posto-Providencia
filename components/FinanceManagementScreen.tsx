...
const FinanceManagementScreen: React.FC = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await api.emprestimo.getAll();
            setLoans(data.map(l => ({
                id: String(l.id),
                credor: l.credor,
    ...
    if (loading && loans.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[600px] w-full text-red-600">
                ...
                <p className="text-gray-500 font-medium text-lg">Carregando dados financeiros...</p>
            </div>
        );
    }
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Gerencie empréstimos, parcelas e compromissos futuros do posto.</p>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-black text-gray-900">R$ {totalDebt.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                        <span className="text-gray-400 font-medium">Total de compromissos ativos:</span>
                        <span className="text-red-600 font-bold">{loans.filter(l => l.ativo).length}</span>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <h3 className="text-2xl font-black text-gray-900">R$ {monthlyInstallments.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                        <span className="text-gray-400 font-medium">Próximo vencimento:</span>
                        <span className="text-orange-600 font-black">{nextVencimento !== 'N/A' ? new Date(nextVencimento).toLocaleDateString('pt-BR') : 'Nenhuma'}</span>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        <h2 className="text-lg font-bold text-gray-900">Empréstimos Ativos</h2>
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                            <input
                                type="text"
                                placeholder="Buscar credor..."
                                className="pl-9 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition-all w-48"
                            />
                        </div>
                    </div>
                    ...
                    {loans.length === 0 ? (
                        <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center">
                            ...
                            <h3 className="text-gray-900 font-bold">Nenhum empréstimo registrado</h3>
                            <p className="text-gray-500 text-sm mt-1">Comece clicando no botão "Novo Empréstimo".</p>
                        </div>
                    ) : (
                        loans.map(loan => (
                            <div
                                ...
                                className={`group relative bg-white p-5 rounded-2xl border-2 transition-all cursor-pointer ${selectedLoan?.id === loan.id ? 'border-red-600 shadow-lg ring-1 ring-red-600/10' : 'border-gray-50 hover:border-gray-200 shadow-sm'}`}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2.5 rounded-xl ${selectedLoan?.id === loan.id ? 'bg-red-600 text-white' : 'bg-gray-50 text-gray-400'}`}>
                                            <ArrowUpRight size={20} />
                                        </div>
                                        <div>
                                            <h4 className="font-black text-gray-900 leading-none">{loan.credor}</h4>
                                    ...
                                    <div className="text-right">
                                        <p className="text-lg font-black text-gray-900">R$ {loan.valorTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                ...
                                <div className="flex justify-between items-center">
                                    <span className="text-xs font-bold text-gray-500">{loan.parcelas?.filter(p => p.status === 'pago').length}/{loan.quantidadeParcelas} parcelas pagas</span>
                    ...
                    {selectedLoan ? (
                        <>
                            <div className="flex items-center justify-between mb-6 pb-6 border-b border-gray-100">
                                <div>
                                    <h3 className="text-xl font-black text-gray-900 tracking-tight">{selectedLoan.credor}</h3>
                            ...
                            <div className="space-y-3 overflow-y-auto flex-1 pr-2 max-h-[600px]">
                                {selectedLoan.parcelas?.map((p) => (
                                    <div
                                        key={p.id}
                                        className={`flex items-center justify-between p-4 rounded-xl border transition-all ${p.status === 'pago' ? 'bg-green-50/30 border-green-100 opacity-75' : 'bg-white border-gray-100'}`}
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={`p-2 rounded-lg font-bold text-sm ${p.status === 'pago' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                                                {String(p.numeroParcela).padStart(2, '0')}
                                            </div>
                                            <div>
                                                <p className={`font-bold ${p.status === 'pago' ? 'text-green-800' : 'text-gray-900'}`}>R$ {p.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                                <p className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                                                    <Calendar size={12} /> {new Date(p.dataVencimento).toLocaleDateString('pt-BR')}
                                                    {p.status === 'pago' && <span className="ml-2 px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-[10px] uppercase font-black tracking-tighter">Pago em {new Date(p.dataPagamento!).toLocaleDateString('pt-BR')}</span>}
                                                </p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {p.status !== 'pago' ? (
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-xs font-bold text-gray-700 hover:bg-green-600 hover:text-white hover:border-green-600 transition-all active:scale-95 shadow-sm"
                                                >
                                                    Pagar Parcela
                                                </button>
                                            ) : (
                            ...
                            <div className="mt-6 pt-6 border-t border-gray-100">
                                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-sm font-bold text-gray-900">R$ {(selectedLoan.taxaJuros || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                                    </div>
                                </div>
                            </div>
                        </>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
                            <div className="size-20 bg-gray-50 text-gray-300 rounded-full flex items-center justify-center mb-4 ring-8 ring-gray-50/50">
                                <CreditCard size={40} />
                            </div>
