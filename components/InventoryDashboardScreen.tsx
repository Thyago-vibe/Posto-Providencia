  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 font-sans animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {items.map((item) => (
          <div key={item.id} className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 flex flex-col gap-4 relative overflow-hidden group">
            {/* Background Icon */}
            <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity pointer-events-none">
              {item.iconType === 'pump' && <Fuel size={120} strokeWidth={1} />}
              {item.iconType === 'leaf' && <Leaf size={120} strokeWidth={1} />}
              {item.iconType === 'truck' && <Truck size={120} strokeWidth={1} />}
            </div>
            ...
            <div className="flex justify-between items-start relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-900">{item.volume.toLocaleString('pt-BR')} L</h3>
                </div>
              </div>
              
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold border
                 ${item.status === 'OK' ? 'bg-[#13ec6d]/10 text-green-700 border-[#13ec6d]/20' : ''}
                 ${item.status === 'BAIXO' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : ''}
                 ${item.status === 'CRÍTICO' ? 'bg-red-100 text-red-700 border-red-200 animate-pulse' : ''}
              `}>
            ...
            <div className="space-y-2 relative z-10">
              <div className="flex justify-between text-xs font-medium">
                <span className="text-gray-400">Capacidade: {item.capacity.toLocaleString('pt-BR')} L</span>
                <span className="text-gray-900">{item.percentage}%</span>
              </div>
              <div className="h-2.5 w-full bg-gray-100 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full
                    ${item.status === 'OK' ? 'bg-[#13ec6d]' : ''}
                    ${item.status === 'BAIXO' ? 'bg-yellow-500' : ''}
                    ${item.status === 'CRÍTICO' ? 'bg-red-500' : ''}
                  `} 
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100 relative z-10">
              {item.status === 'OK' ? (
                <>
                   <Calendar size={18} className="text-gray-400" />
                   {item.code === 'ET' ? (
                     // Mocking the "Consumo alto hoje" logic for Etanol as per design
                     <>
                        <TrendingUp size={18} className="text-gray-400" />
                        <p className="text-xs text-gray-500 font-medium">Consumo alto hoje</p>
                     </>
                   ) : (
                     <p className="text-xs text-gray-500 font-medium">Previsão: <span className="text-gray-900 font-bold">{item.daysRemaining} dias restantes</span></p>
                   )}
                </>
              ) : (
                 <>
                   {item.status === 'BAIXO' ? <AlertTriangle size={18} className="text-yellow-500" /> : <AlertOctagon size={18} className="text-red-500" />}
                   <p className="text-xs text-gray-500 font-medium">Previsão: <span className={`${item.status === 'BAIXO' ? 'text-yellow-600' : 'text-red-600'} font-bold`}>{item.daysRemaining} dias restantes</span></p>
                 </>
              )}
            </div>
          </div>
