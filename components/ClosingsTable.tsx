  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col">
      <div className="p-6 border-b border-gray-100 flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">Fechamentos do Dia</h2>
        <button className="text-sm font-semibold text-blue-700 hover:text-blue-900">Ver todos</button>
      </div>
            ...
            {(!data || data.length === 0) ? (
                ...
                <tr key={closing.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                        <img src={closing.avatar} alt={closing.name} className="w-10 h-10 rounded-full object-cover border border-gray-200" />
                        <span className="font-medium text-gray-900">{closing.name}</span>
                    </div>
                    </td>
                    <td className="px-6 py-4 text-gray-600 text-sm">{closing.shift}</td>
                    <td className="px-6 py-4 font-semibold text-gray-900">
                    R$ {closing.totalSales.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-4 text-center">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ${closing.status === 'OK' ? 'bg-green-100 text-green-800' : ''}
                        ${closing.status === 'Divergente' ? 'bg-red-100 text-red-800' : ''}
                        ${closing.status === 'Aberto' ? 'bg-yellow-100 text-yellow-800' : ''}
                    `}>
